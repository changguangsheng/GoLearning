...
func (xc *XClient) dial(rpcAdr string) (*Client, error) {
	xc.mu.Lock()
	defer xc.mu.Unlock()
	//检查 xc.clients 是否有缓存的 Client，如果有，检查是否是可用状态，如果是则返回缓存的 Client，如果不可用，则从缓存中删除。
	client, ok := xc.clients[rpcAdr]
	if ok && !client.IsAvailable() {
		_ = client.Close()
		delete(xc.clients, rpcAdr)
		client = nil
	}
	if client == nil {
		var err error
		client, err = XDial(rpcAdr, xc.opt)
		if err != nil {
			return nil, err
		}
		xc.clients[rpcAdr] = client
	}
	return client, nil
}
